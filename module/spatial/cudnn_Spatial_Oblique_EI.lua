local Spatial_Oblique_EI, parent =
    torch.class('cudnn.Spatial_Oblique_EI', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function Spatial_Oblique_EI:__init(nInputPlane, nOutputPlane,
    ...
    self.gradWeight = torch.Tensor(nOutputPlane, nInputPlane/self.groups, kH, kW)
     self.eye_matrix=torch.eye(nOutputPlane,nOutputPlane):cuda()
...
function Spatial_Oblique_EI:resetWeightDescriptors(desc)
    -- for compatibility
...
function Spatial_Oblique_EI:fastest(mode)
...
function Spatial_Oblique_EI:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function Spatial_Oblique_EI:resetMode()
    self.fmode = nil
    self.bdmode = nil
    self.bwmode = nil
    return self
...
function Spatial_Oblique_EI:noBias()
   self.bias = nil
   self.gradBias = nil
   return self
end
...
function Spatial_Oblique_EI:checkInputChanged(input)
...
function Spatial_Oblique_EI:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if Spatial_Oblique_EI.checkInputChanged(self, input) then
...
function Spatial_Oblique_EI:updateOutput(input)
    -----------------------------for each training iteration, projection the weight to unit norm
...
function Spatial_Oblique_EI:updateGradInput(input, gradOutput)
...
function Spatial_Oblique_EI:accGradParameters(input, gradOutput, scale)
   ...
   viewGradWeight(self)
    ...
    self.buffer=self.buffer or self.weight.new()
     ...
     self.buffer1:resizeAs(self.eye_matrix):copy(self.eye_matrix)
...
function Spatial_Oblique_EI:clearDesc()
    self.weightDesc = nil
    self.biasDesc = nil
    self.convDesc = nil
    self.iDesc = nil
...
function Spatial_Oblique_EI:write(f)
    self:clearDesc()
    local var = {}
...
function Spatial_Oblique_EI:clearState()
   self:clearDesc()
   nn.utils.clear(self, '_input', '_gradOutput', 'input_slice', 'output_slice')
   return nn.Module.clearState(self)
end

function Spatial_Oblique_EI:updateWeight()
