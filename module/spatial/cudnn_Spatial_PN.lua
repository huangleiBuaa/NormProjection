local Spatial_PN, parent =
    torch.class('cudnn.Spatial_PN', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function Spatial_PN:__init(nInputPlane, nOutputPlane,
...
function Spatial_PN:resetWeightDescriptors(desc)
    -- for compatibility
...
function Spatial_PN:fastest(mode)
...
function Spatial_PN:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function Spatial_PN:checkInputChanged(input)
...
function Spatial_PN:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if Spatial_PN.checkInputChanged(self, input) then
...
function Spatial_PN:updateOutput(input)
...
function Spatial_PN:updateGradInput(input, gradOutput)
...
function Spatial_PN:accGradParameters(input, gradOutput, scale)
...
function Spatial_PN:clearState()
   self:clearDesc()
...
function Spatial_PN:updateWeight()
