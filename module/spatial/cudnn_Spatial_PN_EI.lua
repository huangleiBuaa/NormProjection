local Spatial_PN_EI, parent =
    torch.class('cudnn.Spatial_PN_EI', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function Spatial_PN_EI:__init(nInputPlane, nOutputPlane,
...
function Spatial_PN_EI:resetWeightDescriptors(desc)
    -- for compatibility
...
function Spatial_PN_EI:fastest(mode)
...
function Spatial_PN_EI:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function Spatial_PN_EI:resetMode()
    self.fmode = nil
    self.bdmode = nil
    self.bwmode = nil
    return self
...
function Spatial_PN_EI:checkInputChanged(input)
...
function Spatial_PN_EI:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if Spatial_PN_EI.checkInputChanged(self, input) then
...
function Spatial_PN_EI:updateOutput(input)
...
function Spatial_PN_EI:updateGradInput(input, gradOutput)
...
function Spatial_PN_EI:accGradParameters(input, gradOutput, scale)
...
function Spatial_PN_EI:clearDesc()
    self.weightDesc = nil
    self.biasDesc = nil
    self.convDesc = nil
    self.iDesc = nil
...
function Spatial_PN_EI:write(f)
    self:clearDesc()
    local var = {}
