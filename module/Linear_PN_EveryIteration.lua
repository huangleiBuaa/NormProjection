local Linear_PN_EveryIteration, parent = torch.class('nn.Linear_PN_EveryIteration', 'nn.Module')

function Linear_PN_EveryIteration:__init(inputSize, outputSize,lr, orth_flag)
   parent.__init(self)
...
function Linear_PN_EveryIteration:reset(stdv)
   if stdv then
...
function Linear_PN_EveryIteration:reset_orthogonal()
...
function Linear_PN_EveryIteration:updateOutput(input)
   --self.bias:fill(0)
   ...
   if self.printDetail then
     print("Linear_PN_EveryIteration: input, number fo example=20")
     print(input[{{1,20},{}}]) 
     
     print("Linear_PN_EveryIteration: weight")
     print(self.weight) 
     
     print("Linear_PN_EveryIteration: bias")
     print(self.bias) 
     
     print("Linear_PN_EveryIteration: activation, number fo example=20")
     print(self.output[{{1,20},{}}])  
   end
   return self.output
end

function Linear_PN_EveryIteration:updateGradInput(input, gradOutput)
   if self.gradInput then
    ...
    if self.printDetail then
     print("Linear_PN_EveryIteration: gradOutput, number fo example=20")
...
function Linear_PN_EveryIteration:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:dim() == 1 then
...
function Linear_PN_EveryIteration:updateWeight()
...
Linear_PN_EveryIteration.sharedAccUpdateGradParameters = Linear_PN_EveryIteration.accUpdateGradParameters


function Linear_PN_EveryIteration:__tostring__()
