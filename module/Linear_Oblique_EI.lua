local Linear_Oblique_EI, parent = torch.class('nn.Linear_Oblique_EI', 'nn.Module')

function Linear_Oblique_EI:__init(inputSize, outputSize,lr, orth_flag)
   parent.__init(self)
...
function Linear_Oblique_EI:reset(stdv)
   if stdv then
...
function Linear_Oblique_EI:reset_orthogonal()
...
function Linear_Oblique_EI:updateOutput(input)
   --self.bias:fill(0)
...
function Linear_Oblique_EI:updateGradInput(input, gradOutput)
   if self.gradInput then
    ...
    if self.printDetail then
     print("Linear_Oblique_EI: gradOutput, number fo example=20")
...
function Linear_Oblique_EI:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:dim() == 1 then
  ...
  if torch.type(input)=='torch.CudaTensor' then
      self.buffer1=torch.eye(self.buffer:size(1),self.buffer:size(1)):cuda()
  else
...
Linear_Oblique_EI.sharedAccUpdateGradParameters = Linear_Oblique_EI.accUpdateGradParameters


function Linear_Oblique_EI:__tostring__()
