...
cmd:option('-save',"log_exp_Cifar_NoW" ,'subdirectory to save logs')
...
baseString=opt.dataset_flag..'_'..opt.model..'_depth'..opt.depth
         ..'_lr'..opt.learningRate
...
log_name='Cifar_NoW_'..baseString
...
if not (epoch==opt.max_epoch) then

    for k,v in pairs(model:findModules('cudnn.Spatial_PN')) do
        print('------------projection weight ----------------PN')
        v:updateWeight()
    end
end
...
torch.save('SModel_Cifar_NoW_'..baseString..'.t7', net:clearState())
end
