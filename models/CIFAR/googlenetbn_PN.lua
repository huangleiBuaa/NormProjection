...
require '../../module/spatial/cudnn_Spatial_PN'

local function inception(input_size, config)
   local concat = nn.Concat(2)
   if config[1][1] ~= 0 then
      local conv1 = nn.Sequential()
      conv1:add(cudnn.Spatial_PN(input_size, config[1][1],1,1,1,1)):add(nn.ReLU(true))
   ...
   conv3:add(cudnn.Spatial_PN(  input_size, config[2][1],1,1,1,1)):add(nn.ReLU(true))
   conv3:add(nn.SpatialBatchNormalization(config[2][1],1e-3))
   conv3:add(nn.ReLU(true))
   conv3:add(cudnn.Spatial_PN(config[2][1], config[2][2],3,3,1,1,1,1)):add(nn.ReLU(true))
   conv3:add(nn.SpatialBatchNormalization(config[2][2],1e-3))
   conv3:add(nn.ReLU(true))
   concat:add(conv3)

   local conv3xx = nn.Sequential()
   conv3xx:add(cudnn.Spatial_PN(  input_size, config[3][1],1,1,1,1)):add(nn.ReLU(true))
   conv3xx:add(nn.SpatialBatchNormalization(config[3][1],1e-3))
   conv3xx:add(nn.ReLU(true))
   conv3xx:add(cudnn.Spatial_PN(config[3][1], config[3][2],3,3,1,1,1,1)):add(nn.ReLU(true))
   conv3xx:add(nn.SpatialBatchNormalization(config[3][2],1e-3))
   conv3xx:add(nn.ReLU(true))
   conv3xx:add(cudnn.Spatial_PN(config[3][2], config[3][2],3,3,1,1,1,1)):add(nn.ReLU(true))
   conv3xx:add(nn.SpatialBatchNormalization(config[3][2],1e-3))
   conv3xx:add(nn.ReLU(true))
   concat:add(conv3xx)
   ...
   if config[4][2] ~= 0 then
      pool:add(cudnn.Spatial_PN(input_size, config[4][2],1,1,1,1)):add(nn.ReLU(true))
...
--   features:add(cudnn.Spatial_PN(3,64,7,7,2,2,3,3))
--   features:add(nn.SpatialBatchNormalization(64,1e-3))
--   features:add(nn.ReLU(true))
--   features:add(nn.SpatialMaxPooling(3,3,2,2):ceil())
--   features:add(cudnn.Spatial_PN(64,64,1,1))
--   features:add(nn.SpatialBatchNormalization(64,1e-3))
--   features:add(nn.ReLU(true))
   features:add(cudnn.Spatial_PN(3,64,3,3,1,1,0,0))
   features:add(nn.SpatialBatchNormalization(64,1e-3))
   features:add(nn.ReLU(true))
   features:add(cudnn.Spatial_PN(64,192,3,3,1,1,0,0))
...
--   features:add(nn.SpatialMaxPooling(3,3,2,2):ceil())
   ...
   features:add(cudnn.Spatial_PN(576,576,2,2,2,2))
   ...
   main_branch:add(cudnn.Spatial_PN(1024,1024,2,2,2,2))
...
--   aux_classifier:add(cudnn.Spatial_PN(576,128,1,1,1,1))
